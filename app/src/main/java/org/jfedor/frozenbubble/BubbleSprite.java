...
public class BubbleSprite extends Sprite {
  ...
  public void move() {
    ...
    if (checkCollision(neighbors) || realY < 44.+frozen.getMoveDown()) {
      ...
      if(frozen.isRainbow) this.setBestColor(neighbors);
      Vector<Sprite> checkJump = new Vector<Sprite>();
      this.checkJump(checkJump, neighbors);

      if (checkJump.size() >= 3 || frozen.isRainbow) {
        released = true;
  ...
  public final void paint(Canvas c, double scale, int dx, int dy) {
    ...
    if (blink && bubbleFace != frozenFace) {
      ...
      if(frozen.isRainbow && !fixed) drawImage(frozen.bubbleRainbow, p.x, p.y, c, scale, dx, dy);
      else if (FrozenBubble.getMode() == FrozenBubble.GAME_NORMAL ||
          bubbleFace == frozenFace) {
  ...
  public void setBestColor(Vector<BubbleSprite> neighbors) {
    int[] matches = new int[frozen.bubbles.length];
    Vector<BubbleSprite> tempNeighbors;
    Vector<Sprite> tempCheckJump;

    for(int i=0; i<matches.length; i++) {
      refreshJumpCheck();
      tempNeighbors = new Vector<BubbleSprite>();
      tempCheckJump = new Vector<Sprite>();
      for(BubbleSprite n : neighbors) tempNeighbors.add(n);

      this.changeBubbleColor(i);
      this.checkJump(tempCheckJump, tempNeighbors);
      matches[i] = tempCheckJump.size();
    }

    refreshJumpCheck();
    this.changeBubbleColor(findMaxIndex(matches));
  }
  ...
  private void changeBubbleColor(int color) {
    this.bubbleFace = frozen.bubbles[color];
    this.color = color;
  }

  private void refreshJumpCheck() {
    BubbleSprite[][] grid = frozen.getGrid();
    for(BubbleSprite[] bubbleLine : grid) {
      for(BubbleSprite bubble : bubbleLine) {
        if(bubble != null) bubble.checkJump = false;
      }
    }
  }
