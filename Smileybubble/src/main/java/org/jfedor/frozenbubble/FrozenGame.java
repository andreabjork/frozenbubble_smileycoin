...
public class FrozenGame extends GameScreen {
  ...
  BmpWrap background;
  BmpWrap background_frozen;
  BmpWrap bubbleRainbow;
  BmpWrap[] bubbles;
  ...
  public FrozenGame(BmpWrap background_arg,
    ...
    launchBubble = new LaunchBubbleSprite(currentColor, 
                                          ...
                                          launcher, bubbles, bubblesBlind, bubbleRainbow);
  ...
  public FrozenGame(BmpWrap background_arg,
                    ...
                    LevelManager levelManager_arg,
                    HighscoreManager highscoreManager_arg) {
    this(background_arg, background_frozen_arg, bubbles_arg, bubblesBlind_arg, frozenBubbles_arg,
         targetedBubbles_arg, bubbleBlink_arg, null, gameWon_arg, gameLost_arg,
  ...
  public gameEnum play(boolean key_left, boolean key_right,
    ...
    if (!endOfGame && isArcade && !isFrozen) {
      scrollBubbles();
    }
  ...
  public boolean drop() {
    for(int i=0; i<LevelManager.NUM_COLS; i++) {
      int j = bottomRowIdx(i);
      if(bubblePlay[i][j]!=null) {
        addFallingBubble(bubblePlay[i][j]);
        bubblePlay[i][j].removeFromManager();
        bubblePlay[i][j] = null;
      }
    }

    // this is to make sure there are no lonely bubbles left hanging around with nothing attached.
    for(int i=0; i<LevelManager.NUM_COLS; i++) {
      for(int j=0; j<LevelManager.NUM_ROWS; j++) {
        if(bubblePlay[i][j]!=null) {
          int count = 0;
          for(BubbleSprite neighbor : bubblePlay[i][j].getNeighbors(bubblePlay[i][j].currentPosition())) if(neighbor != null) count++;
          if(count == 0) {
            addFallingBubble(bubblePlay[i][j]);
            bubblePlay[i][j].removeFromManager();
            bubblePlay[i][j] = null;
  ...
  public boolean explode() {
    ...
    launchBubble.setRainbow(true);
    isRainbow = true;
    return true;
  }

  private int bottomRowIdx(int column) {

    for(int i=LevelManager.NUM_ROWS-1; i>=0; i--) {
      if(bubblePlay[column][i] != null) return i;
    }
