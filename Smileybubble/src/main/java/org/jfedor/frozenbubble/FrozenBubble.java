...
import com.tutorweb.frozenbubble.Inventory;
import com.tutorweb.frozenbubble.ProductManager;
import com.tutorweb.frozenbubble.StoreActivity;
...
public class FrozenBubble extends Activity
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if(numPlayers == 1)updateProductPanel();
  }
...
//-------------------------------- Added power-up options:  ---------------------------------------
  ...
  // Keeps track of powerups in user's inventory.
  public void updateProductPanel() {
    Map<String, ?> allProducts = inventory.getAllInventory();
    ProductManager PM = new ProductManager(this);
    String[] ID = PM.getAvailableProdIds();

    // Update the textview
    TextView[] views = new TextView[]{(TextView)findViewById(R.id.product_label1), (TextView)findViewById(R.id.product_label2), (TextView)findViewById(R.id.product_label3)};
    for(int i = 0; i<views.length; i++) {
      String id = ID[i];
      ((ViewGroup)views[i].getParent()).setTag(id);
      views[i].setText(id + " " + Integer.toString(inventory.getProductAmount(id)));
    }
  }

  public void useProduct(View view) {
    boolean success = false;
    String prodId = ((ViewGroup)view.getParent()).getTag().toString();
    if(inventory.containsProduct(prodId)) {
      if(prodId.equals("Freeze")) success = mGameThread.powerFreeze();
      else if(prodId.equals("Drop")) success = mGameThread.powerDrop();
      else if(prodId.equals("Explode")) success = mGameThread.powerExplode();

      if(success) inventory.spendProduct(prodId);
    } else {
      Log.i("STORE", "Out of product! Go to store.");
      pause();
      openStoreDialog();
    }
    updateProductPanel();
  ...
  private void openStoreDialog() {
    ...
    storeDialogBuilder
            .setTitle("Out of powerup!")
            .setMessage("Go to the store to buy more power-ups for smileycoins!")
            .setCancelable(true)
            .setPositiveButton("Take me to Store!", new DialogInterface.OnClickListener() {
              public void onClick(DialogInterface dialog, int id) {
            ...
            .setNegativeButton("Continue game", new DialogInterface.OnClickListener() {
              public void onClick(DialogInterface dialog, int id) {
                mGameThread.setState(stateEnum.RUNNING);
                dialog.cancel();
              }
            });
