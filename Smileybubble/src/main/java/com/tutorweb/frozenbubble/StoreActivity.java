...
public class StoreActivity extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState != null) {
            for(int i=0; i<mPagerAdapter.getCount(); i++) {
                mPagerAdapter.getItem(i).setAmount(savedInstanceState.getInt(Integer.toString(i)));
            }

            updateProductList();
            updateButton();
            mPager.setCurrentItem(savedInstanceState.getInt("INDEX"));
        }
    ...
    public void purchase(View view) {
        try {
            ...
            openPurchaseErrorDialog();
        }
    }

    private void openPurchaseErrorDialog() {
        AlertDialog.Builder storeDialogBuilder = new AlertDialog.Builder(this);
        storeDialogBuilder
                .setTitle("No wallet installed")
                .setMessage("You need to have the smileycoin wallet installed on your device to complete the purchase.")
                .setCancelable(false)
                .setPositiveButton("Get the wallet!", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        try {
                            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=hashengineering.smileycoin.wallet")));
                        } catch (ActivityNotFoundException e) {
                            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://play.google.com/store/apps/details?id=de.schildbach.wallet")));
                        }
                    }
                })
    ...
    private class ProductPagerAdapter extends FragmentStatePagerAdapter {
        ...
        public ProductPagerAdapter(FragmentManager fm, List<StoreFragment> fragments) {
            ...
            this.fragments.get(0).hideLeftArrow();
            this.fragments.get(getCount()-1).hideRightArrow();
        }
